...
// Verifica se o payload é válido - Formato BrutalCash
...
try {
    // Extrair os dados relevantes do formato BrutalCash
    $transactionId = $event['Id'];
    $status = $event['Status'];
    $amount = isset($event['Amount']) ? intval($event['Amount'] * 100) : 0; // Converte de reais para centavos
    $paymentMethod = $event['PaymentMethod'] ?? 'pix';
    $createdAt = $event['CreatedAt'] ?? date('Y-m-d H:i:s');
    $updatedAt = $event['UpdatedAt'] ?? date('Y-m-d H:i:s');
    ...
    error_log("[Webhook] 💰 Valor: R$ " . number_format($amount/100, 2, ',', '.') . " (" . $amount . " centavos)");
    error_log("[Webhook] 📅 Criado em: " . $createdAt);
    error_log("[Webhook] 📅 Atualizado em: " . $updatedAt);
    if ($paidAt && $paidAt !== '0001-01-01T00:00:00') {
        error_log("[Webhook] 💳 Pago em: " . $paidAt);
    }
    ...
    // Mapeia os status da BrutalCash para o formato interno
    $statusMap = [
        'PENDING' => 'pending',
        'PAID' => 'paid',
        'REFUNDED' => 'refunded',
    ...
    if ($status === 'PAID') {
        ...
        if ($pedido) {
            ...
            // Usa dados do banco e do webhook BrutalCash
            ...
            $paidAtFormatted = ($paidAt && $paidAt !== '0001-01-01T00:00:00') ? $paidAt : $updatedAt;
            ...
            error_log("[Webhook] 📊 Dados finais - PaidAt: " . $paidAtFormatted);
        ...
        error_log("[Webhook] ℹ️ Status não é PAID, pulando processamento em background. Status atual: " . $status);
    }
