...
try {
    ...
    if (strtolower($status) === 'paid' || strtolower($status) === 'approved') {
        ...
        if ($pedido) {
            ...
            // Valida e corrige datas - se a data do webhook for inválida ou muito antiga, usa a data atual
            $createdAtRaw = $transaction['createdAt'] ?? $pedido['created_at'];
            $paidAtRaw = $transaction['paidAt'] ?? $transaction['createdAt'] ?? date('Y-m-d H:i:s');
            
            // Converte para timestamp e valida
            $createdAtTimestamp = strtotime($createdAtRaw);
            $paidAtTimestamp = strtotime($paidAtRaw);
            $currentTimestamp = time();
            
            // Se a data for no futuro ou mais de 1 ano no passado, usa a data atual
            if ($createdAtTimestamp === false || $createdAtTimestamp > $currentTimestamp || $createdAtTimestamp < ($currentTimestamp - 31536000)) {
                $createdAt = date('Y-m-d H:i:s');
                error_log("[Webhook] ⚠️ Data createdAt inválida ou fora do range, usando data atual: " . $createdAt);
            } else {
                $createdAt = date('Y-m-d H:i:s', $createdAtTimestamp);
            }
            
            if ($paidAtTimestamp === false || $paidAtTimestamp > $currentTimestamp || $paidAtTimestamp < ($currentTimestamp - 31536000)) {
                $paidAt = date('Y-m-d H:i:s');
                error_log("[Webhook] ⚠️ Data paidAt inválida ou fora do range, usando data atual: " . $paidAt);
            } else {
                $paidAt = date('Y-m-d H:i:s', $paidAtTimestamp);
            }
            ...
            error_log("[Webhook] 📊 Dados extraídos - CreatedAt: " . $createdAt);
            ...
            // Envia para utmify.php usando o mesmo scriptDir do otimizey
